...
it('creates no tables beyond the Phase 5 persistence set', async () => {
  ...
  expect(rows.results.map(({ name }) => name).filter((name) => !name.startsWith('sqlite_') && !name.startsWith('_cf_')).sort())
    .toEqual(['admission_policies', 'conversations', 'd1_migrations', 'default_conversations', 'messages', 'processed_updates', 'provider_credentials', 'providers', 'request_admissions', 'users']);
});
