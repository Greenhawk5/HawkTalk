...
| Duplicate update double-processing | `update_id` idempotency table + Phase 6 durable processing states: generation starts only on the atomic claimed→generating transition; AI failure/uncertainty is terminal per update | 2, 6 |
| Repeat AI generation after crash/timeout | `markGenerating` is durable BEFORE the provider call; a generating/failed/completed update is never regenerated by redelivery — completed updates reuse the persisted assistant message | 6 |
| Cross-user conversation reuse via groups | only `private` chats enter the conversational pipeline; group/supergroup/channel/unknown chats are acknowledged without user upsert, conversation resolution, history, AI, or persistence | 6 |
| Telegram chat ID as conversation key | chat IDs are never used as conversation identifiers; `default_conversations` is keyed by internal `users.id` only | 6 |
...
| `CREDENTIAL_MASTER_SECRET` | `wrangler secret put` | human |
| LLM API keys | sealed envelopes in D1 (`provider_credentials`), decrypted only in-router via the master secret | human/admin |
| Search API key (later phase) | `wrangler secret put` | human |
